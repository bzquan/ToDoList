...
test.describe('ドラッグ&ドロップで並び替え', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 3つの項目を追加（unshiftのためDOMは[項目C, 項目B, 項目A]）
  ...
  test('ドラッグ&ドロップで順序が入れ替わる', async ({ page }) => {
    ...
    await expect(displayed.nth(1)).toContainText('項目A');
    await expect(displayed.nth(2)).toContainText('項目C');
  });

  test('localStorage に順序が保存される', async ({ page }) => {
    const items = page.locator('.todo-item');
    // 項目C（DOM先頭）をドラッグ元として取得
    const draggedId = await items.nth(0).getAttribute('data-id');
  ...
  test('削除後もドラッグハンドルは表示される', async ({ page }) => {
    ...
    const items = page.locator('.todo-item');
    // 削除後: DOMは[項目C, 項目A]
    await expect(items.nth(0)).toContainText('項目C');
